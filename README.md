...
결과 — 5탭 네비게이션
  ...
  ├── 밥메이트   7차원 유사도 TOP 3 + 🤖 AI 매칭 코멘트 (Bedrock)
...
| 중복 닉네임 방지 | `Map`으로 최신 ts 기준 1명 유지 |
| 연결 상태 표시 | `.info/connected` 리스너 |
| 좋아요 동시 집계 | `transaction()` 원자적 처리 |
| 실시간 참여자 동기화 | `on("value")` 리스너 |
...
| 담당 | 내용 |
|---|---|
| ChoHyeonChan | Firebase 백엔드 / 실시간 동기화 / 채팅 / Bedrock AI / S3 배포 |
| junseok0929 | 취향 알고리즘 / 맛 슬라이더 / 로그아웃 / 좋아요 실시간 반영 |
| juneddoha | GPS 식당 추천 / 3단계 위저드 / 5탭 결과 UI |
